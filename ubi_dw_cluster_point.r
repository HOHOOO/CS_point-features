...
trip<-sql(hiveContext,paste0("SELECT * , LEAD(actual_start, 1, 0) OVER (PARTITION BY deviceid ORDER BY actual_start) AS start2 FROM trip_stat where stat_date like'",date_period,"%'"))
...
SparkR:::saveAsTextFile(end_rdd_value, paste0("/user/kettle/ubi/dw/cluster_point/stat_date=",date_period,"01"))
sql(hiveContext,"drop TABLE ubi_dw_cluster_point_temp")
sql(hiveContext,paste0("CREATE external TABLE ubi_dw_cluster_point_temp (deviceid String,tid String,vid String,start String,actual_start String,s_end String,dura DOUBLE,period String,lat_st_ori DOUBLE,lon_st_ori DOUBLE,lat_en_ori DOUBLE,lon_en_ori DOUBLE,m_ori DOUBLE,lat_st_def DOUBLE,lon_st_def DOUBLE,lat_en_def DOUBLE,lon_en_def DOUBLE,m_def DOUBLE,speed_mean DOUBLE,gps_speed_sd DOUBLE,gps_acc_sd DOUBLE,dura2 String,sort_st String,sort_en String,dim_month string) ROW format delimited FIELDS TERMINATED BY ',' LOCATION '/user/kettle/ubi/dw/cluster_point/stat_date=",date_period,"01'"))
sql(hiveContext,"insert overwrite table ubi_dw_cluster_point_temp select trim(deviceid) as deviceid,trim(tid) as tid,trim(vid) as vid,trim(start) as start,trim(actual_start) as actual_start,trim(s_end) as s_end,dura,trim(period) as period,lat_st_ori,lon_st_ori,lat_en_ori,lon_en_ori,m_ori,lat_st_def,lon_st_def,lat_en_def,lon_en_def,m_def,speed_mean,gps_speed_sd,gps_acc_sd,trim(dura2) as dura2,trim(sort_st) as sort_st,trim(sort_en) as sort_en,trim(dim_month) as dim_month from ubi_dw_cluster_point_temp")
sql(hiveContext,paste0("ALTER TABLE ubi_dw_cluster_point add PArtition (stat_date='",date_period,"01')"))
